	...
	public void clear() {
		for (int x = 0; x < this.w; ++x) {
			for (int y = 0; y < this.h; ++y) {
				this.zBuffer[x][y] = 1.0D;
			}
		}

	}

	private void drawTriangle(Point3D pA, Point3D pB, Point3D pC, int color, Boolean mode) {
		Vec3D a = pA.dehomog().get();
		Vec3D b = pB.dehomog().get();
		Vec3D c = pC.dehomog().get();
		if (Math.min(Math.min(a.x, b.x), c.x) <= 1.0D && Math.max(Math.max(a.x, b.x), c.x) >= -1.0D
				&& Math.min(Math.min(a.y, b.y), c.y) <= 1.0D && Math.max(Math.max(a.y, b.y), c.y) >= -1.0D
				&& Math.min(Math.min(a.z, b.z), c.z) <= 1.0D && Math.max(Math.max(a.z, b.z), c.z) >= 0.0D) {
			...
			b = b.mul(new Vec3D(1.0D, -1.0D, 1.0D)).add(new Vec3D(1.0D, 1.0D, 0.0D))
					.mul(new Vec3D((double) ((this.w - 1) / 2), (double) ((this.h - 1) / 2), 1.0D));
			c = c.mul(new Vec3D(1.0D, -1.0D, 1.0D)).add(new Vec3D(1.0D, 1.0D, 0.0D))
					.mul(new Vec3D((double) ((this.w - 1) / 2), (double) ((this.h - 1) / 2), 1.0D));
			...
			if (mode) {
				...
				for (y = Math.max((int) a.y + 1, 0); (double) y <= Math.min(b.y, (double) (this.h - 1)); ++y) {
					...
					for (x = Math.max((int) x1 + 1, 0); (double) x <= Math.min(x2, (double) (this.w - 1)); ++x) {
						t = ((double) x - x1) / (x2 - x1);
						z = z1 * (1.0D - t) + z2 * t;
						if (this.zBuffer[x][y] > z && z > 0.0D) {
							this.zBuffer[x][y] = z;
				...
				for (y = Math.max((int) b.y + 1, 0); (double) y <= Math.min(c.y, (double) (this.h - 1)); ++y) {
				...
				this.drawMyLine(g, a.x, a.y, b.x, b.y, color);
				this.drawMyLine(g, a.x, a.y, c.x, c.y, color);
				this.drawMyLine(g, c.x, c.y, b.x, b.y, color);
			}
